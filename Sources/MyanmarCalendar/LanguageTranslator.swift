...
/// Language Translator
///
/// Port of the Java `mmcalendar.LanguageTranslator`.
///
/// Catalog index order: 0 = English, 1 = Myanmar (Unicode), 2 = Zawgyi, 3 = Mon, 4 = Tai, 5 = Karen (Sgaw)
///
/// Credits:
/// - Mon: ITVilla (http://it-villa.blogspot.com/), proofread by Mikau Nyan
/// - Tai: Jao Tai Num, Tai Dictionary (https://www.taidictionary.com/)
public struct LanguageTranslator {

    private static let langCount = 6
    ...
    /// Trie nodes are keyed by UTF-16 code unit, not by `Character`.
    ///
    /// Swift's grapheme clustering would merge a space with a following Myanmar
    /// spacing vowel sign (UAX #29 GB9a) into a single `Character`, which hides the
    /// start of the next word from the matcher. Java walks `char` values, so we do too.
    private final class TrieNode {
        var children: [UInt16: TrieNode] = [:]
        var translation: [UInt16]?
        var isLeaf: Bool { translation != nil }
    }

    /// Direct word translation maps: [from][to] -> [sourceWord: targetWord]
    ///
    /// Catalog order matters: a source word that appears more than once in a column
    /// (for example Myanmar "နေ့", which is both "Nay" and "Day") resolves to the
    /// *last* catalog row, matching Java's `map.put`.
    private static let directMap: [[[String: String]]] = {
        var maps = Array(repeating: Array(repeating: [String: String](), count: langCount), count: langCount)
        for row in catalog {
            for from in 0..<langCount {
                let source = row[from]
                guard !source.isEmpty else { continue }
                for to in 0..<langCount {
                    maps[from][to][source] = row[to]
                }
            }
        }
        return maps
    ...
    /// Trie roots for longest-prefix sentence translation: [from][to]
    ///
    /// Rows are inserted longest source first (a stable sort by UTF-16 length, as in
    /// Java) and a later insertion overwrites an earlier one, so duplicate source
    /// words resolve to the last catalog row.
    private static let trieRoot: [[[UInt16: TrieNode]]] = {
        var roots = Array(
            repeating: Array(repeating: [UInt16: TrieNode](), count: langCount),
            count: langCount
        )
        for from in 0..<langCount {
            // Stable sort by source length, descending
            let sortedCatalog = catalog.enumerated()
                .sorted { lhs, rhs in
                    let l = lhs.element[from].utf16.count
                    let r = rhs.element[from].utf16.count
                    return l != r ? l > r : lhs.offset < rhs.offset
                }
                .map { $0.element }

            for to in 0..<langCount {
                var root: [UInt16: TrieNode] = [:]
                for row in sortedCatalog {
                    let key = Array(row[from].utf16)
                    guard !key.isEmpty else { continue }

                    var node: TrieNode
                    if let existing = root[key[0]] {
                        node = existing
                    } else {
                        node = TrieNode()
                        root[key[0]] = node
                    ...
                    for i in 1..<key.count {
                        if let child = node.children[key[i]] {
                            node = child
                        } else {
                            let child = TrieNode()
                    ...
                    node.translation = Array(row[to].utf16)
                }
                roots[from][to] = root
            }
        }
    ...
    /// Pre-cached digit translations: [digit][language]
    private static let digits: [[String]] = {
        var result = Array(repeating: Array(repeating: "", count: langCount), count: 10)
        for digit in 0..<10 {
            let digitStr = String(digit)
            for lang in 0..<langCount {
                for row in catalog where row[0] == digitStr {
                    result[digit][lang] = row[lang]
                    break
                }
            }
        }
    ...
    /// Translates a word/phrase from English to the target language.
    /// - Parameters:
    ///   - str: the word to translate
    ///   - to: target language
    /// - Returns: translated word, or the original if not found in the catalog
    public static func translate(_ str: String, _ to: Language) -> String {
        return translate(str, .english, to)
    }

    /// Translates a single word/phrase from one language to another.
    /// - Parameters:
    ///   - str: the word to translate
    ///   - from: source language
    ///   - to: target language
    /// - Returns: translated word, or the original if not found in the catalog
    public static func translate(_ str: String, _ from: Language, _ to: Language) -> String {
        return directMap[from.languageIndex][to.languageIndex][str] ?? str
    }
    ...
    public static func translateSentence(_ text: String, _ from: Language, _ to: Language) -> String {
        if text.isEmpty { return text }

        let root = trieRoot[from.languageIndex][to.languageIndex]
        ...
        while i < len {
            ...
            while j < len {
                guard let next = node.children[units[j]] else { break }
                node = next
                j += 1
                if node.isLeaf {
                    matchEnd = j
            ...
            if let bestTranslation = bestTranslation {
                result.append(contentsOf: bestTranslation)
                i = matchEnd
            } else {
                result.append(c)
                i += 1
    ...
    public static func translateSentenceList(_ sentences: [String], _ from: Language, _ to: Language) -> [String] {
        return sentences.map { translateSentence($0, from, to) }
    }

    // MARK: - Number translation

    /// Converts a number to its string representation in the target language.
    /// - Parameters:
    ///   - number: number to translate (truncated toward zero)
    ///   - language: target language
    /// - Returns: number as a string in the target language
    public static func translate(_ number: Double, _ language: Language) -> String {
        let langIdx = language.languageIndex
        ...
        while num > 0 {
            let digit = num % 10
            result = digits[digit][langIdx] + result
            num /= 10
        }
    ...
    /// Converts an integer to its string representation in the target language.
    public static func translate(_ number: Int, _ language: Language) -> String {
        return translate(Double(number), language)
    }

    // MARK: - Utilities

    /// The number of translation entries in the catalog.
    public static var catalogSize: Int { catalog.count }
}
